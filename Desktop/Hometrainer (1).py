...
# Other vars in code required for interactive things (audio and wind)
wind_strength = 1
volume = 1
...
def feedback_button_func(time_s):
    ...
    while True: # Loop until everything has been played
        ...
        if((channel2.get_busy() == 0) & (counter == 0)):
            counter += 1
            if(time_m == 0): # if timer is 0, we have nothing for 0, so play 1
                play_sound("1",channel)
            elif( time_m > 12):
                play_sound("12",channel)
            else:
                play_sound(f"{time_m}",channel) # else play timer
            print(counter)
...
while True:
    ...
    # wind slider (working)
    wind_slider = 1-slider_value_calculator(adc.read( Wind_slider_pin)) # read out wind slider
    ...
    # feedback button
    #print("feedback: " + str(GPIO.input(feedback_button)))
    if(GPIO.input(feedback_button) != prev_feedback_button): # if pressed and new
        prev_feedback_button = GPIO.input(feedback_button)
        feedback_button_func(currenttime)
        print("feedback: " + str(GPIO.input(feedback_button))) 
    
    # skip button
    if(GPIO.input(skip_button) != prev_skip_button): # if pressed and new 
         print("skip: " + str(GPIO.input(skip_button))) 
         prev_skip_button = GPIO.input(skip_button)
